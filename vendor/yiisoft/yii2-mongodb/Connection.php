...
class Connection extends Component
{
    ...
    public function open()
    {
        if ($this->mongoClient === null) {
            ...
            try {
                ...
                preg_match('/^mongodb:\\/\\/.+\\/([^?&]+)/s', $this->dsn, $matches);
                $dsn = str_replace('/'.$matches[1],'',$this->dsn);
                //$this->mongoClient = new \MongoClient($dsn,array('connect' => true));
                $this->mongoClient = $this->getMongoClient($dsn,array('connect' => true));
    ...
    /*以下来源于网络，连接失败，自动重连 3 次*/
    public function getMongoClient($seeds = "", $options = array(), $retry = 3) {
        try {
